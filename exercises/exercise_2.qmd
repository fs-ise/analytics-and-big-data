...
Because k-means is sensitive to scale differences, the next step is to standardize these two variables before clustering.

---

### 2. Apply K-Means clustering

We try different values of `k` and inspect the resulting cluster structure visually:
...
A value around **k = 4** is likely appropriate.

---

### 3. Add cluster labels to the dataset

After selecting a suitable number of clusters, we store the resulting labels in the original dataframe:
...
We can now compare `revenue` across the identified clusters using a grouped boxplot:
