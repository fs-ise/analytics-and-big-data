...
Here, `department_IT` and `department_Sales` show how those departments differ from the reference department, which is the department that was dropped during one-hot encoding. If `drop_first=True` was used and the departments are ordered alphabetically, the reference category is likely `HR`.
...
Create a simple residual plot for the model.
...
* Compare and subgroup (department-specific) models
...
Before you wrap up, please complete the Session 4 survey here: [{{< var sessions.session_4.survey_url >}}]({{< var sessions.session_4.survey_url >}}){target="_blank"}. Thank you 🙏
:::
